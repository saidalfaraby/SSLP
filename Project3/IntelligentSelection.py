...
class IntelligentSelection(object):
  ...
  def select(self, threshold=None, is_update=False, retrieve_per_iteration=None, n_iteration=1):
    ...
    self.log('unigram : '+str(is_ugram))
    self.log('bigram : '+str(is_bgram))
    if theshold!=None and not is_update:
      ...
      for i in range(n_iteration):
        ...
        for j in range(retrieve_per_iteration):
          self.Selected_Docs.append(sorted_Mix_Docs[j]['docID'])
...
if __name__ == '__main__':
  ...
  In_Model = Features()
  Mix_Model = Features()
  In_Model.load('legal_in_model_noprune.pickle')
  Mix_Model.load('legal_sample_mix_model_noprune.pickle')
